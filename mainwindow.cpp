    ...
    , ui(new Ui::MainWindow)
{
    ...
    bossCombox = 50, bossComboy = 175, bossComboDimensionx = 20, bossComboDimensiony = 30, bossComboMaxDimensionx = 200;
    ...
    filterLabelx = width - sidePanelDimensionx, filterLabely = 0, filterLabelDimensionx = sidePanelDimensionx, filterLabelDimensiony = 40;
    ...
    for(int i=0;i< monster->monsterList.size();i++)
    {
        ...
        bossUI.checkbox->setStyleSheet(
            "QCheckBox { font-size: 16px; color: black; spacing: 8px; }"
        ...
        for(int c = 0; c < monster->numCC; c++)
        {
            ...
            for(int m = 0; m < std::get<2>(monster->monsterList[i]).size(); m++)
            {
                ...
                connect(std::get<0>(bossUI.timerList[c][m]), &QTimer::timeout, this, [=]() { timerUpdate(i, c, std::get<4>(listBossUI[i].timerList[c][m])); });
            }
        }
    ...
    {
        std::get<1>(listBossUI[std::get<3>(it)].timerList[std::get<4>(it)][std::get<5>(it)]) = std::get<0>(it);
        std::get<2>(listBossUI[std::get<3>(it)].timerList[std::get<4>(it)][std::get<5>(it)]) = std::get<1>(it);
        std::get<3>(listBossUI[std::get<3>(it)].timerList[std::get<4>(it)][std::get<5>(it)]) = std::get<2>(it);
        std::get<4>(listBossUI[std::get<3>(it)].timerList[std::get<4>(it)][std::get<5>(it)]) = std::get<5>(it);

        if(std::get<5>(it) == mapMonster[std::get<3>(it)])
            processTimer(std::get<0>(it), std::get<1>(it), std::get<2>(it), std::get<3>(it), std::get<4>(it), std::get<5>(it));

        std::get<0>(listBossUI[std::get<3>(it)].timerList[std::get<4>(it)][std::get<5>(it)])->start(1000);
    }
...
void MainWindow::timerButtonClick(QAbstractButton* button)
{
    ...
    if(buttonName.left(7) == "button2")
    {
        ...
        case QMessageBox::Yes:
            ...
            std::get<0>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]])->stop();
            std::get<1>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]) = QDateTime();
    ...
    else if(buttonName.left(7) == "button1" && !std::get<0>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]])->isActive())
    {
        ...
        std::get<1>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]) = boundTime;
        std::get<2>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]) = lowerBoundTime;
        std::get<3>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]) = upperBoundTime;
        std::get<4>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]) = mapMonster[bossIndex];

        processTimer(boundTime, lowerBoundTime, upperBoundTime, bossIndex, ccIndex, std::get<4>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]]));

        std::get<0>(listBossUI[bossIndex].timerList[ccIndex][mapMonster[bossIndex]])->start(1000);
...
void MainWindow::timerUpdate(int bossIndex, int ccIndex, int mapIndex)
{
    ...
    qDebug() << std::get<1>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]).toString("hh:mm");

    if(mapIndex == mapMonster[bossIndex])
        processTimer(std::get<1>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]),
                 std::get<2>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]),
                 std::get<3>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]), bossIndex, ccIndex, mapIndex);
}
...
void MainWindow::changeMap(const QString& map, int bossIndex)
{
    ...
    for(int c = 0; c < monster->numCC; c++)
    {
        if(std::get<1>(listBossUI[bossIndex].timerList[c][mapMonster[bossIndex]]) != QDateTime())
        {
            processTimer(std::get<1>(listBossUI[bossIndex].timerList[c][mapMonster[bossIndex]]),
                         std::get<2>(listBossUI[bossIndex].timerList[c][mapMonster[bossIndex]]),
                         std::get<3>(listBossUI[bossIndex].timerList[c][mapMonster[bossIndex]]), bossIndex, c, mapMonster[bossIndex]);
        }
        else
        {
...
void MainWindow::changeDisplayTime()
{
    ...
    for(int i=0;i< monster->monsterList.size();i++)
        for(int c = 0; c < monster->numCC; c++)
            if(std::get<0>(listBossUI[i].timerList[c][mapMonster[i]])->isActive())
                processTimer(std::get<1>(listBossUI[i].timerList[c][mapMonster[i]]),
                             std::get<2>(listBossUI[i].timerList[c][mapMonster[i]]),
                             std::get<3>(listBossUI[i].timerList[c][mapMonster[i]]), i, c, mapMonster[i]);
}
...
void MainWindow::processTimer(QDateTime boundTime, QDateTime lowerBoundTime, QDateTime upperBoundTime, int bossIndex, int ccIndex, int mapIndex)
{
    ...
    if(displayTime)
    {
        ...
        if(curTime.secsTo(upperBoundTime) > 0)
        {
            ...
            std::get<1>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]) = boundTime;
            std::get<2>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]) = lowerBoundTime;
            std::get<3>(listBossUI[bossIndex].timerList[ccIndex][mapIndex]) = upperBoundTime;
        }
        else
        {
            listBossUI[bossIndex].timer2BossCC[ccIndex]->setText("");
            ...
            std::get<0>(listBossUI[bossIndex].timerList[ccIndex][mapIndex])->stop();
        }
    }
    else
    {
